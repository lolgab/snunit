// Copy pasted from tapir Netty implementation
...
private[tapir] class FilterServerEndpoints[R, F[_]](rootLayer: PathLayer[R, F])
    extends (ServerRequest => List[ServerEndpoint[R, F]]) {

  /** Given a request, returns the list of server endpoints which might potentially decode successfully, taking into
    * account the path of the request.
    */
...
object FilterServerEndpoints {
  ...
  private def createLayer[R, F[_]](
      segmentsToEndpoints: List[(List[PathSegment], ServerEndpoint[R, F])]
  ): PathLayer[R, F] = {
